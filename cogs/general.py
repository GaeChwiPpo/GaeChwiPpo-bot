...
class General(commands.Cog):
    ...
    async def server_info(self, ctx):
        ...
        embed = discord.Embed(
            title=f"📊 {guild.name} 서버 정보", color=discord.Color.blue()
        )
        ...
        embed.add_field(
            name="생성일", value=guild.created_at.strftime("%Y-%m-%d"), inline=True
        )
    ...
    async def user_info(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"👤 {member.display_name} 정보", color=member.color
        )
        ...
        embed.add_field(
            name="사용자명", value=f"{member.name}#{member.discriminator}", inline=True
        )
        ...
        embed.add_field(
            name="서버 가입일", value=member.joined_at.strftime("%Y-%m-%d"), inline=True
        )
    ...
    async def clear_messages(self, ctx, amount: int = 5):
        ...
        await ctx.send(
            f"✅ {len(deleted) - 1}개의 메시지를 삭제했습니다.", delete_after=3
        )
